    ...
    std::vector<NetworkInterface> NetworkMonitor::getNetworkInfo()
    {
        ...
        for (struct ifaddrs *ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next)
        {
            ...
            if (family == AF_INET || family == AF_INET6)
            {
                ...
                if (family == AF_INET)
                {
                    inet_ntop(AF_INET, &reinterpret_cast<struct sockaddr_in *>(ifa->ifa_addr)->sin_addr, addr,
                              sizeof(addr));
                }
                else
                {
                    inet_ntop(AF_INET6, &reinterpret_cast<struct sockaddr_in6 *>(ifa->ifa_addr)->sin6_addr, addr,
                              sizeof(addr));
                }
                iface.ip = addr;
            }
            ...
            for (auto &iface : interfaces)
            {
                ...
                if (it != previousStats.end())
                {
                    ...
                    if (elapsedMs > 0)
                    {
                        ...
                        const unsigned long long rxDiff =
                            iface.rxBytes >= it->second.rxBytes ? iface.rxBytes - it->second.rxBytes : 0;
                        const unsigned long long txDiff =
                            iface.txBytes >= it->second.txBytes ? iface.txBytes - it->second.txBytes : 0;
